...
2. **Probe → product** — If System ops shows a capability (`run_submission`, `session_fork`), Spark must either use it or stop advertising it as “ready.”
...
**Status (2026-07-11):** Done (flagged + hardened) — cancel→`POST …/stop`, toolsets/repo/reasoning forwarded in submit body, approve bridge route. Residual: MoA on runs SSE; gateway may ignore some forwarded fields; repo proposal approval stays agent-loop.
...
**Verify:** Toggle runs on; send chat; see tools stream; stop works; approval works; MoA still works.
...
**Status (2026-07-11):** Done (core) — 8.1 session fork, 8.2 checkpoint restore, 8.3 worktree toggle (chat + kanban; residual: GitHub API tools / cleanup), 8.4 fallback toast, 8.5 `/resume` attach.
...
**Verify:** Fork → continue; restore file from checkpoint; worktree path used for a card; fallback toast on forced failure.
...
**Status (2026-07-11):** Done — MCP unified (sidebar + Settings → hermes-api / config.yaml); credential pool; projects switcher + bind-board UI; formation routing; Fleet swarm labels. Residual: zustand MCP APIs remain unused in store for legacy code only.
...
| **9.2** Credential pool UI | Thin Settings panel: list/add/reset via `hermes auth` proxy | Rotate OpenRouter/Nous keys without editing `auth.json` by hand |
...
**Verify:** Install MCP in sidebar appears in agent; auth list matches CLI; project bind affects board cwd.
...
**Status (2026-07-11):** Done — all planned P2/P3 surfaces shipped. Leftover hardening also done: Portal device-code OAuth in Settings; `/v1/runs` slim body + parity/worktree/computer_use gates (honest agent-loop fallback); CU running frames + capture_after hint + broken progress path removed.
...
| MoA on runs | P2 | Opt-in translate | Done (`HERMES_RUNS_MOA` / capability) |
...
_Historical — all items below have been implemented in-tree as of 2026-07-11; use for commit/PR splitting if desired._
...
9. **PR-I** — `/v1/runs` behind flag (bridge translate)
10. **PR-J** — Worktree launch + fallback toast
...
| Layer | Coverage |
|-------|----------|
| Unit | Adapter selection in kanban runner; checkpoint restore message builders; formation→backend map; MCP list normalization |
| Bridge | Adapter init smoke for kanban env; runs event → existing event types; fork returns new session id |
| UI | Profiles save round-trip; restore sheet; MCP sidebar matches store; runs toggle streams tools |
| Regression | Non-MoA agent-loop; MoA picker; native kanban CRUD; teams restart from JSON store |
| E2E (optional) | Flag runs on → approve dangerous tool → stop mid-run |
...
| Gateway capability honesty | If `runs=yes` shown, either used (flag) or labeled “probe only” |
...
- Claim Phase “Done” for slash-passthrough-only features

---

## 10. Immediate recommendation

**Start with Phase 6 (PR-A + PR-B).**  
It is the smallest change that removes the biggest honesty gap (composer vs fleet brains) and fixes obvious stubs, without the risk of a full transport migration.

**Then Phase 8.1–8.2 (fork + checkpoint restore)** — high user value, medium size.

**Then Phase 7 (`/v1/runs`)** once adapter is the only brain — largest payoff, highest risk; keep behind a flag.
...
Update §14 of the 0.18 plan when Phase 6+ PRs land: mark “surfaces Done / execution deferred” → “execution converging.”
